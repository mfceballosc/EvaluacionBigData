...
if __name__ == "__main__":
    ...
    _df_i = leer_archivo_comuna(file_path)
    _df_cust = pd.read_parquet(file_customers)
    _df_empl = pd.read_parquet(file_employees)
    _df_comunas = pd.read_parquet(file_comunas_path) 
    ...
    for cont, fecha in enumerate(fechas):
        ...
        df_i = _df_i.copy()
        df_cust = _df_cust.copy()
        df_empl = _df_empl.copy()
        df_comunas = _df_comunas.copy()
